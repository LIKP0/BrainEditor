...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument(
        ...
        default="configs/train_oasis2.yaml",
    )
    parser.add_argument(
        "-t",
        "--train",
...
class ImageLogger(Callback):
    def __init__(self, batch_frequency, max_images, clamp=True, increase_log_steps=True,
                 rescale=True, disabled_log_image=False, log_on_batch_idx=False, log_first_step=False,
        ...
        self.clamp = clamp
        self.disabled = disabled_log_image
...
if __name__ == "__main__":
    ...
    try:
        ...
        trainer = Trainer.from_argparse_args(trainer_opt, plugins=DDPPlugin(find_unused_parameters=False),
                                             **trainer_kwargs)
        ...
        if not cpu:
            ...
            # if type(lightning_config.trainer.gpus) == str:
            #     ngpu = len(lightning_config.trainer.gpus.strip(",").split(','))
            # else:
            #     ngpu = 1
        else:
